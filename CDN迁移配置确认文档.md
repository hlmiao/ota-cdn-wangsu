...
| 项 | 取值 / 说明 |
| --- | --- |
| 演练加速域名 | `ota-wangsu-test.qa.c3.volvocars.com.cn`（已 ICP 备案，主域名 `volvocars.com.cn`）|
...
> 实测：演练节点返回的证书 `subject=CN=ota-wangsu-test.qa.c3.volvocars.com.cn`，HTTPS 握手成功。
...
| 项 | 取值 / 说明 |
| --- | --- |
| S3 桶 | `ota-wangsu-firmware`，区域 **cn-north-1（北京）**，aws-cn 分区，账号 685095924131 |
| 桶访问性 | **私有桶**（Block all public access），SSE-AES256 |
| 源站名（CDN Pro）| `aws_origin`（与 Edge Logic `origin_pass aws_origin` 对应）|
| 源站主机名 | `ota-wangsu-firmware.s3.cn-north-1.amazonaws.com.cn` |
| **Host 请求头** | 必须填**桶域名**（同上），否则 S3 返回 `404 NoSuchBucket` |
| 回源协议 | HTTPS + 回源 SNI |
| 回源鉴权 | **AWS S3（SigV4）**，地区 **cn-north-1** |
| 鉴权凭证 | IAM 用户 `wangsu-origin-ota`（最小权限 `s3:GetObject` on `arn:aws-cn:s3:::ota-wangsu-firmware/*`）；**SK 存入 CDN Pro Secrets**，AccessKey 明文填 |
...
| 用例 | 结果 |
| --- | --- |
| HTTPS 握手 / 证书 | 返回证书 `CN=ota-wangsu-test.qa.c3.volvocars.com.cn`，握手成功 ✅ |
...
| N4 | **子域备案接入** | `ota-wangsu-test.qa.c3.volvocars.com.cn`（及未来生产子域）是否需在网宿侧单独做**接入备案** |
...
| C7 | **回源凭证** | 是否沿用 IAM 用户 `wangsu-origin-ota` 最小权限策略；密钥轮换归属 |
...
| 项 | 值 |
| --- | --- |
| 演练加速域名 | `ota-wangsu-test.qa.c3.volvocars.com.cn` |
| 演练节点 IP | `211.91.233.246` / `222.246.147.73`（取自控制台 Staging IP 列表）|
| S3 桶 / 区域 | `ota-wangsu-firmware` / cn-north-1 |
| 测试对象 | `firmware/v1.2.3/app.bin`（2 MB，etag `2710c7a8…`）|
| 回源 IAM 用户 | `wangsu-origin-ota`（AccessKey/SK 详见配置，**待轮换**）|
